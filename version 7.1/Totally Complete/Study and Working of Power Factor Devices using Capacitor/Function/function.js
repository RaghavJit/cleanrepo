...
document.getElementById("body").onmousemove = function () {
    if (mouseDown) {
        var x = window.event.clientX-677;
        var y = 453-window.event.clientY;
        var angleOfRoation;

        if((x>0)&&(y>0)){
            angleOfRoation = (Math.atan(x/y)*180)/Math.PI
        }
        else if((x<0)&&(y>0)){
            angleOfRoation = ((Math.atan(x/y)*180)/Math.PI - 360)
            console.log("error")
        }
        else if((x<0)&&(y<0)){
            angleOfRoation = (Math.atan(x/y)*180)/Math.PI - 180
        }
        else if((x>0)&&(y<0)){
            angleOfRoation = (180-((Math.atan(x/y)*180)/Math.PI))*(-1)
        }
        
        console.log(angleOfRoation)
        rotate_element(angleOfRoation, knob)
    }
}
...
check.onclick = function chkConn() {
    ...
    if ((arrChk == 11) && (index != 3) && (instance.getAllConnections().length == 11)) {
...
variacSlider.oninput = function updateValsCall() {
    ...
    else if ((mcb_state == 1) && (var_state == 1) && (onOff_state == 1)) {
        toggle_load_on();
    }
}
