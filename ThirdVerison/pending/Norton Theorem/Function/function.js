...
var valconn3 = [p_pow, c_p_pow, n_pow, c_n_pow, c_lr, c_ll, c_ul, c_ur, c_ur, p_v, c_lr, n_v]
...
function updateAmmeters(n) {
    ...
    Isc = (parseFloat(variacSlider.value) / R) * (r2 / (r3 + r2))
...
check.onclick = function callCheck() {
    ...
    for (var i = 0; i < 3; i++) {
        ...
        else if ((i == 2) && (checkcon(valList[i])) == 6) {
            window.alert("Load and Voltmeter connected")
            caseVal = 3;
        }
    }
...
add.onclick = function AddToTable() {
    ...
    let R_n = row.insertCell(1);
    let Isc_n = row.insertCell(2);
    let V_n = row.insertCell(3)
    let Reading = row.insertCell(4);

    SNo.innerHTML = obs + 1;

    R_n.innerHTML = (R).toPrecision(3);
    Isc_n.innerHTML = (Isc).toPrecision(3);
    V_n.innerHTML = (V).toPrecision(3);
    Reading.innerHTML = document.getElementById("Rls").value;
    obs = obs + 1;
